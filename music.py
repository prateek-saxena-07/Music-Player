...
#font is defined which is to be used for button font

defined_font= font.Font(family='Helvetica')

#play button
play_button=Button(root,text="▶",width=7,command=Play,borderwidth=0)  #borderwith=0 for rounded corners
play_button['font']=defined_font
play_button.grid(row=1,column=0)

#pause button
pause_button= Button(root,text="⏸",width=7 ,command=Pause,borderwidth=0)
pause_button['font']=defined_font
pause_button.grid(row=1,column=1)

#stop button

stop_button= Button(root,text="⏹",width=7 ,command=Stop,borderwidth=0)
stop_button['font']=defined_font
stop_button.grid(row=1,column=2)

#resume button

resume_button= Button(root,text="⏯",width=7 ,command=Resume,borderwidth=0)
resume_button['font']=defined_font
resume_button.grid(row=1,column=3)

#next button

next_button= Button(root,text="⏭",width=7 ,command=Next,borderwidth=0)
next_button['font']=defined_font
next_button.grid(row=1,column=4)

#previous button
previous_button= Button(root,text="⏮",width=7 ,command=Previous,borderwidth=0)
